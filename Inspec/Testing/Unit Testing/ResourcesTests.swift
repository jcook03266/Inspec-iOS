...
final class ResourcesTests: XCTestCase {
    ...
    func testAllVideosExist() throws {
        for video in VideoRepository.allCases {
            XCTAssertNoThrow(Video(from: video.rawValue))
            XCTAssertNoThrow(Video(from: video.rawValue).loadResourceURLFromBundle())
        }
    }
